...
- NOTE-19: with a low CTL the weekly target can sit below the load of the sessions already on the calendar; the proposal is then fillers only. The header should say so explicitly ("your planned sessions already cover the target"). Follow-up.
- NOTE-20: the target-zone chip on plan cards uses a profile-only zone model (resting HR default 60 → "Z1 · 60–132 bpm") while Zones & paces uses the measured resting HR (48 → "48–133 bpm"); unify by giving the plan cards the same bounds. Follow-up.
